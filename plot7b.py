...
def visualize_combined(time_our, time_cgbn, time_bnbp, nodes, continuous,
    ...
    for i, vertex in enumerate(nodes):
        ...
        for entry in time_our:
            if entry['info'][0] == vertex and entry['info'][1] == 0.5:
                evids = entry['info'][2]
                x_vals.append(evids)
    ...
    for n in range(len(nodes)):
        y_vals = []
        for e in range(10):
            y_vals.append(time_cgbn[n, 2, e])
        plot_data(x_vals, y_vals, 'CGBayesNets',
                  colors[1], line_styles[1], markers[1])

    # Plot BayesNetBP times
    plot_data(x_vals, time_bnbp, 'BayesNetBP',
    ...
    plt.grid(False)
    plt.yscale('log')
    # plt.savefig(f'{target_dir}/comparison2.png', format='png', dpi=600)
    plt.savefig(f'{target_dir}/comparison2.eps', format='eps', dpi=600)
    plt.show()
...
if __name__ == "__main__":
    ...
    file_bnbp = os.path.join(source3, 'Time_V50C50.txt')
    num_networks = 100
    nodes = [50]
    continuous = [0.5]
