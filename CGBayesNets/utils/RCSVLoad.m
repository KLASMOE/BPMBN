...
i = 1;
while (~done)
    if (length(remain) > 1 && remain(1) == '"' && remain(2) == '"') 
        % empty column name:
        isquoted(i) = true;
        remain = remain(3:end);
        tok = '';        
    elseif (length(remain) > 2 && remain(2) == '"' && remain(3) == '"')
        % empty column name:
        isquoted(i) = true;
        remain = remain(4:end);
        tok = '';
    elseif (length(remain) > 1 && (remain(1) == '"' || remain(2) == '"'))
        isquoted(i) = true;
        [tok, remain] = strtok(remain(2:end),'"');
        % eat the final doublequote character
        remain = remain(2:end);
        % if what's left is a delimiter, we should eat it
        % otherwise the leading delim looks like a missing value
        for d = 1:length(delims)
            if (strcmp(remain(1),delims(d)) && (length(remain) > 1))
                remain = remain(2:end);
            end
        end
    else
        emptytok = false;
        for d = 1:length(delims)
            % worse case: if there's one char left and it's a delimiter,
            % that means there's TWO empty tokens:
            if (strcmp(remain(1),delims(d)) && length(remain) == 1)
                emptytok = true;
                tok = [];
                remain = remain(2:end);
                % this adds the extra token here
                dline{end+1} = tok;
                break;
            end
            if (strcmp(remain(1),delims(d)))            
            % this logic seems unnecessary
%             if ((strcmp(remain(1),delims(d)) && i == 1) ||... % picks up first case missing value
%                 (strcmp(remain(1),delims(d)) && length(remain) == 1) ||... % picks up last case missing value
%                 (strcmp(remain(1),delims(d)) && strcmp(remain(2),delims(d))))
                emptytok = true;
                tok = [];
                remain = remain(2:end);
                break;                
        ...
        if (~emptytok)
            % this leaves an extra delimiter at the front:
            [tok, remain] = strtok(remain, delims);
            if (length(remain) > 1)
                remain = remain(2:end);
            elseif (~isempty(remain))
                % ends with empty token:
                remain = [];
                dline{end+1} = tok;
                tok = [];
            end
...
for i = 1:length(dline)
    ...
    if (isnan(numres))
        nums(i) = false;
        if (isempty(dline{i}) && LEADING_NA_ASNUMERIC)
            nums(i) = true;
        elseif (strcmp('NA',dline{i}) && LEADING_NA_ASNUMERIC)
            nums(i) = true;
        elseif (strcmp('NaN',dline{i}) && LEADING_NA_ASNUMERIC)
            nums(i) = true;
        end
    end
end
