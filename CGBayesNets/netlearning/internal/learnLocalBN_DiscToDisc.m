...
childConfig = unique(childData(~isnan(childData)),'legacy');
...
% set up joint parent-child data by dropping rows with NaNs in them

% drop rows that have any NaNs in the child data
if (~isempty(parentData))
    parentData = parentData(:,~isnan(childData));
end
childData = childData(~isnan(childData));
