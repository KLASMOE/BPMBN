function [auc,MBNet,BNet,predAccuracy, predClass, predZs] = BNLearn(data, cols, pheno, ...
    algorithm, BF_THRESH, verbose, priorPrecision, learn)
%[auc,MBNet] = BFFilterBNLearn(data, cols, pheno, algorithm, BF_THRESH, verbose, priorPrecision, learn)
%
% Main function for building a CG BayesNet.  
% 
% This will then learn a network using one of 5 possible algorithms,
% each one based on the BF of the variables 
...
%       edge at each step.
%   ALGORITHM = 4 : basic flat Naive Bayes network.
%   ALGORITHM = 5 : Naive Bayes network augmented with a tree structure.
...
%           each node
% LEARN: if true, will also learn the network and get an AUC.  optional.
%   Default = true.
%
% OUTPUT:
...
if (algorithm == 4)
    if (verbose)
        fprintf(1,'Learning Naive Bayes Network\n');
    end
    BNet = NBBayesNet(data, cols, pheno, BF_THRESH, 'NaiveBayes', priorPrecision);    
end
if (algorithm == 5)
    if (verbose)
        fprintf(1,'Learning Tree-Augmented Naive Bayes Network\n');
    end
    BNet = TreeAugmentedNB(data, cols, pheno, BF_THRESH, 'TreeAugNB', priorPrecision);
end
...
if (learn)
    ...
    [predAccuracy, predClass, predZs] = PredictPheno(MBNet,verbose);
