...
for i = 1:length(thresh)
    ...
    [acc, p, z] = PredictPheno(MBNetParams, verbose);
    convexhullAUC(i) = AUCWorker(acc,p,z,MBNetParams.GetPhenoCol(),true);

    fprintf(1,'Predicting on Testing Data\n');
    MBNetParams = MBNetParams.ReplaceData(testdata,testcols);
    [testacc, testp, testz] = PredictPheno(MBNetParams, verbose);
    testconvexhullAUC(i) = AUCWorker(testacc,testp,testz,MBNetParams.GetPhenoCol(),true);
