...
learn = true;
[auc,MBNet] = BFFilterBNLearn(winedata,winecols,'class',1,0,true,priorPrecision,learn);
...
[auc,MBNet] = BFFilterBNLearn(winedata,winecols,'class',2,0,true,priorPrecision,learn);
...
[auc,MBNet] = BFFilterBNLearn(winedata,winecols,'class',3,0,true,priorPrecision,learn);
...
%% test some missing-value stuff:
BFTHRESH = 0;

[numdata, cols] = RCSVLoad('MissingData_v3.csv',false,',',false,true);
pheno = cols{1};
alg = 1;
[auc,MBNet,BNet] = BNLearn(numdata, cols, pheno, alg, BFTHRESH, verbose);

%%
[numdata, cols] = RCSVLoad('HybridData_v1.csv',false,',',false,true);
pheno = cols{1};
alg = 1;
[auc,MBNet,BNet] = BNLearn(numdata, cols, pheno, alg, BFTHRESH, verbose);

%%
[numdata, cols] = RCSVLoad('MissingData_v6.csv',false,',',false,true);
pheno = cols{1};
for alg = 1:5
    [auc,MBNet,BNet] = BNLearn(numdata, cols, pheno, alg, BFTHRESH, verbose);
end

%% do some work with the Bank dataset:
[numdata, cols, ~, ncolinds] = RCSVLoad('bank2.csv',false,',',false,true);

pheno = 'y';
ncols = cols(ncolinds);
for i = 1:5
    alg = i;
    % test each of the five network search algorithms
    [auc,MBNet,BNet,algacc{i},algp{i},algz{i}] = BNLearn(numdata, ncols, pheno, alg, BFTHRESH, verbose);
    algptrue{i} = MBNet.GetPhenoCol();
    % combine into a AUC multi plot:
end
legendstrings = {'K2','PhenoCentric','Exhaustive','NaiveBayes','TreeNB'};
doRawROC = false;
aucmultiplot(algp, algz, algptrue, legendstrings, doRawROC);
title('Bank Dataset, Algorithm Comparison');
% shows K2 provides best AUC.

%% do some CV with the Bank data:
% also test bootstrap ballance
bffilter = true;
randfolds = false;
foldsbyclass = true;
bootstrapballance = true;
folds = 5;
CVModelLearnEx(numdata, ncols, pheno, priorPrecision, folds, 'BankCVTest', ...
    verbose, randfolds, 1, bffilter, foldsbyclass, bootstrapballance);


%%
% other things to potentially test:
% dynamic BNs
% missing phenotypes
% prediciton of all vars simultaneously
% PermLearn
% difference between ModelLearnAndTest and BNLearnAndTest
% BFFilterBNLearn
% BayesianInteraction
