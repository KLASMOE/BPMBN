...
%%
% i think this actually works with NaNs layered in the way we want, by
% ignoring those rows (because it uses a test of equality, which NaNs fail 
% by defalt):
...
if (size(d,1) > size(cpt.table,1))    
    for i = 1:size(cpt.table,1)
        ...
        totalobserved = totalobserved + sum(zs == 0);
    end
else
    for i = 1:size(d,1)
        ...
        totalobserved = totalobserved + sum(zs);
    end    
end

%% generally don't bother with normalizing anything
% DEBUGGING: This is normalizing every pair of rows in the log prob table,
% which seems to be an error based on an assumption that the CPT would be 
% very small. 
%a = sum(cpt.logprob);
...
if (DEBUGGING)
    ...
    totalobserved = totalobserved + priorPrecision.alpha;
    cpt.logprob = cpt.logprob / totalobserved;
end
