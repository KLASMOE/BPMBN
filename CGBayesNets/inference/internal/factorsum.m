...
%
% This will SUM OUT "VARNAME", removing it from the CPT
%
...
for i = 1:length(vals)
    ...
    if (i > 1)
        % need to do this with logplus() : 
        lpstoadd = sparse(cpt.logprob(inds));
        for j = 1:length(lpstoadd)
            newprob(j) = logplus([newprob(j), lpstoadd(j)]);
        end
    else
        newprob = sparse(cpt.logprob(inds));
    end
end
...
%% set up new CPT:
newf = CondProbTable(cpt.node, {cpt.factors{finds}}, newtable, newprob, {cpt.values{finds}}, cpt.findex(finds),[]);
