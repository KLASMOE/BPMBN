...
%% probably here: filter out rows with NaNs:
%jointd = [data,selfdata];
%jointd = jointd(sum(isnan(jointd),2) == 0,:);
% actually we can just filter the NaNs when we use the data to call
% learnlocal_ContToCont(), since before that we're using NaNs in tests for
% equality, which they fail, and then get left out by default
...
for d = 1:numvalcombos
    ...
    % also need to skip to the ELSE clause if there's no y data remaining
    % in selfdata that isn't NaN:
    if (sum(zs == 0) > 0 && sum(~isnan(selfdata(zs == 0))) > 0)
        y = selfdata(zs == 0);
        ...
        localModel = learnLocalBN_ContToCont([],mean(selfdata(~isnan(selfdata))),priorPrecision);
        b = localModel.regreCoeff;
        %var_y = localModel.postVar;
        var_y = 1/localModel.postPrec;
        if (var_y < 0)
