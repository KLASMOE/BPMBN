...
for i = 1:ncases
    ...
    % marginalize over remaining discrete vars (which may have had no
    % evidencce entered on them)
    sumcpt = MultiFactorSum(csttree.cpt,pheno);
    
    % find where the phenotype is in the factors of SUMCPT
    phnmatches = strcmpi(sumcpt.factors, nodes(phenind).self);
    rootphenoind = find(phnmatches);
    
    [maxval, mind] = max(sumcpt.logprob);
    z(i) = exp(maxval - logplus(sumcpt.logprob));
    
    % predcition is the highest value:
    p(i) = sumcpt.values{rootphenoind}(mind);
    if (verbose)
        fprintf(1,'\t prediction for example %d is : %d with confidence %1.2f ',i,p(i), z(i));
        if (isnan(drow(phencol)))
            fprintf(1,' no pheno given!\n');
        elseif (drow(phencol) == p(i))
            fprintf(1,' right!\n');
        else
            fprintf(1,' wrong!\n');
        end
...
acc = (p(~isnan(data(:,phencol))) == data(~isnan(data(:,phencol)),phencol));
acc = sum(acc) / length(data(~isnan(data(:,phencol)),phencol));
