...
time <- vector("numeric", length = length(E))

for (v in V) {
  for (c in C) {
    file_name <- paste("Time_V", v, "C", c, ".txt", sep="") # 保存运行时间的文件
    if (file.exists(file_name)) {
      file.remove(file_name)  # 如果文件存在则删除
    }
    #cat(sprintf("%dC%d\n", v, c), file = file_name, append = TRUE)
    ...
    for (e in E) {
      ...
      if (e==0) {
        for (p in P) {
          ...
          # pdf(paste(output_path, "plot", v, "C", c, "_", p, ".pdf", sep=""))
      ...
      # 将每次循环的时间和平均时间写入文件
      # cat(sprintf("V=%d, C=%.2f, N=%d, E=%d: Times = %s (s), Average Time = %s (s)\n", v, c/100, n, e,
...
data <- data.frame(x = E, y = mean_time)

# 使用 ggplot 绘制时间折线图，添加颜色映射以生成图例
ggplot(data, aes(x = x, y = y)) +
  geom_line(color = "blue") +  # 绘制线条
  geom_point(shape = 21, size = 3, fill = "red", color = "black") +  # 更改点的样式 +  # 添加点
  geom_text(aes(label = sprintf("%.3f", y)), nudge_y = 0.01) +  # 添加文本标签，使用 sprintf 格式化为三位小数
  labs(title = paste0("传播计算平均时间随证据变量变化折线（V=", V, ",C=", C/100, ",N=", N, "）"), x = "证据变量个数", y = "平均时间(seconds)") +  # 添加标签和图例标题
